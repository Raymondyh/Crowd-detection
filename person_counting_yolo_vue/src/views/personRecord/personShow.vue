	...
	<div class="system-predict-container layout-padding">
		<div class="system-predict-padding layout-padding-auto layout-padding-view">
			<div class="header">
				<div class="model">
					<text>模型：</text>
					<el-input v-model="state.form.model" style="width: 120px" size="large" disabled />
				</div>
				<div class="height" style="margin-left: 10px">
					<text>高度：</text>
					<el-input v-model="state.form.height" style="width: 120px" size="large" disabled />
				</div>
				<div class="username" style="margin-left: 10px">
					<text>用户：</text>
					<el-input v-model="state.form.username" style="width: 120px" size="large" disabled />
				</div>
				<div class="inCount" style="margin-left: 10px">
					<text>进入人数：</text>
					<el-input v-model="state.form.inCount" style="width: 120px" size="large" disabled />
				</div>
				<div class="outCount" style="margin-left: 10px">
					<text>离开人数：</text>
					<el-input v-model="state.form.outCount" style="width: 120px" size="large" disabled />
				</div>
				<div class="startTime" style="margin-left: 10px">
					<text>开始时间：</text>
					<el-input v-model="state.form.startTime" style="width: 240px" size="large" disabled />
				</div>
			...
			<div class="cards" ref="cardsContainer">
				...
				<div class="left" :style="{ width: leftWidth + '%' }">
					<video class="video" v-if="state.form.videoPath" preload="auto" controls>
						<source :src="state.form.videoPath" type="video/mp4" />
					</video>
				</div>
				...
				<div class="right" :style="{ width: 100 - leftWidth + '%' }">
					<video class="video" preload="auto" v-if="state.form.uploadedUrl" controls>
						<source :src="state.form.uploadedUrl" type="video/mp4" />
					</video>
				</div>
			</div>
		</div>
...
const getData = () => {
	request.get('/api/personrecords/' + state.id).then((res) => {
		if (res.code == 0) {
